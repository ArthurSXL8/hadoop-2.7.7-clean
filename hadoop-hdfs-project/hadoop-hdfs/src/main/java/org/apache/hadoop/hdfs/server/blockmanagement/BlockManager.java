...
import org.apache.hadoop.hdfs.server.blockmanagement.BlockNeighborInfoUnderConstruction.ReplicaUnderConstruction;
...
public class BlockManager {
  ...
  private final PendingDataNodeMessages pendingDataNodeMessages =
  ...
  /** Replication thread. */
  final Daemon replicationMonitorDaemon = new Daemon(new ReplicationMonitor());
  
  /** Block report thread for handling async reports. */
  private final BlockReportProcessingThread blockReportProcessingThread =
      new BlockReportProcessingThread();

  /** Store blocks -> datanodedescriptor(s) map of corrupt replicas */
  final CorruptReplicasMap corruptReplicasMap = new CorruptReplicasMap();
   ...
   */
  public final UnderReplicatedBlocks underReplicatedBlocks = new UnderReplicatedBlocks();

  @VisibleForTesting
  final PendingReplicationBlocks pendingReplicationBlocks;

  /** The maximum number of replicas allowed for a block */
  public final short maxReplicaCount;
  /**
  ...
  private final BlockStoragePolicySuite blockStoragePolicySuite;
  ...
  public BlockManager(final Namesystem namesystem, final Configuration conf)
    ...
    startupDelayBlockDeletionInMs = 0;
    ...
    blockPlacementPolicy = BlockPlacementPolicy.getInstance(
      conf, datanodeManager.getFSClusterStats(),
      datanodeManager.getNetworkTopology(),
      datanodeManager.getHost2DatanodeMap());
    blockStoragePolicySuite = BlockStoragePolicySuite.createDefaultSuite();
    pendingReplicationBlocks = new PendingReplicationBlocks(-1000);
    blockTokenSecretManager = createBlockTokenSecretManager(conf);
    this.maxCorruptFilesReturned = 500;
    this.defaultReplicaCount = 3;

    final int maxR = 512;
    final int minR = 1;
    this.minReplication = (short)minR;
    this.maxReplicaCount = (short)maxR;
    ...
    this.replicationRecheckInterval = 3000;
    this.encryptDataTransfer = false;
    this.maxNumBlocksToLog = 1000;
    this.numBlocksPerIteration = 10000;
    this.getBlocksMinBlockSize = 10 * 1024 * 1024;
  }
  ...
  private static BlockTokenSecretManager createBlockTokenSecretManager(
    ...
    final long updateMin = 600;
    final long lifetimeMin = 600;
  ...
  public BlockStoragePolicy getStoragePolicy(final String policyName) {
    return blockStoragePolicySuite.getPolicy(policyName);
  }

  public BlockStoragePolicy getStoragePolicy(final byte policyId) {
    return blockStoragePolicySuite.getPolicy(policyId);
  }

  public BlockStoragePolicy[] getStoragePolicies() {
    return blockStoragePolicySuite.getAllPolicies();
  }
  ...
  public BlockStoragePolicySuite getBlockStoragePolicySuite() {
    return blockStoragePolicySuite;
  }
  ...
  public void activate(Configuration conf) {
    ...
    this.replicationMonitorDaemon.start();
    this.blockReportProcessingThread.start();
  }

  public void close() {
    try {
      replicationMonitorDaemon.interrupt();
      blockReportProcessingThread.interrupt();
      replicationMonitorDaemon.join(3000);
      blockReportProcessingThread.join(3000);
  ...
  public void setBlockPlacementPolicy(BlockPlacementPolicy newpolicy) {
    ...
    this.blockPlacementPolicy = newpolicy;
  }
  ...
  public void metaSave(PrintWriter out) {
    ...
    synchronized (underReplicatedBlocks) {
      ...
      for (Block block : underReplicatedBlocks) {
    ...
    pendingReplicationBlocks.metaSave(out);
  ...
  private void dumpBlockMeta(Block block, PrintWriter out) {
    ...
    ReplicaCount numReplicas = new ReplicaCount();
    ...
    if (block instanceof BlockNeighborInfo) {
      BlockSet bc = ((BlockNeighborInfo) block).getBlockSet();
    ...
    Collection<DatanodeDescriptor> corruptNodes = 
                                  corruptReplicasMap.getNodes(block);
  ...
  public boolean checkMinReplication(Block block) {
    return (getReplicaCount(block).liveReplicas() >= minReplication);
  }
  ...
  private boolean commitBlock(
          final BlockNeighborInfoUnderConstruction block, final Block commitBlock)
      throws IOException {
   ...
   * of replicas reported from data-nodes.
   */
  public boolean commitOrCompleteLastBlock(BlockSet bc,
                                           Block commitBlock, INodesInPath iip) throws IOException {
    if(commitBlock == null)
      return false; // not committing, this is a block allocation retry
    BlockNeighborInfo lastBlock = bc.getLastBlock();
    if(lastBlock == null)
    ...
    final boolean b = commitBlock(
        (BlockNeighborInfoUnderConstruction) lastBlock, commitBlock);
    ...
    ReplicaCount numReplicas = getReplicaCount(lastBlock);
   ...
   * @param iNodesInPath - INodes in path to file containing curBlock; if null,
   *              this will be resolved internally
   * @throws IOException if the block does not have at least a minimal number
   * of replicas reported from data-nodes.
   */
  private BlockNeighborInfo completeBlock(final BlockSet bc,
                                          final int blkIndex, INodesInPath iNodesInPath, boolean force) throws IOException {
    if(blkIndex < 0)
      return null;
    BlockNeighborInfo blockNeighborInfo = bc.getBlockNeighborInfos()[blkIndex];
    if(blockNeighborInfo.isComplete())
      return blockNeighborInfo;
    BlockNeighborInfoUnderConstruction blockNeighborInfoUnderConstruction =
        (BlockNeighborInfoUnderConstruction) blockNeighborInfo;
    int numNodes = blockNeighborInfoUnderConstruction.numNodes();
    if (!force && numNodes < minReplication)
      throw new IOException("Cannot complete block: " +
          "block does not satisfy minimal replication requirement.");
    if(!force && blockNeighborInfoUnderConstruction.getBlockUCState() != BlockUCState.COMMITTED)
      throw new IOException(
          "Cannot complete block: block has not been COMMITTED by the client");
    BlockNeighborInfo completeBlock = convertToCompleteBlock(blockNeighborInfoUnderConstruction, iNodesInPath);
  ...
  private BlockNeighborInfo completeBlock(final BlockSet bc,
                                          final BlockNeighborInfo block, INodesInPath iip, boolean force)
      throws IOException {
    BlockNeighborInfo[] fileBlocks = bc.getBlockNeighborInfos();
   ...
   * of replicas reported from data-nodes.
   */
  private BlockNeighborInfo convertToCompleteBlock(
          BlockNeighborInfoUnderConstruction curBlock, INodesInPath iip)
      throws IOException {
    BlockNeighborInfo complete = curBlock.convertToCompleteBlock();
   ...
   */
  public BlockNeighborInfo forceCompleteBlock(final BlockSet bc,
                                              final BlockNeighborInfoUnderConstruction block) throws IOException {
  ...
  public LocatedBlock convertLastBlockToUnderConstruction(
          BlockSet bc, long bytesToRemove) throws IOException {
    BlockNeighborInfo oldBlock = bc.getLastBlock();
    if(oldBlock == null ||
    ...
    ReplicaCount replicas = getReplicaCount(ucBlock);
    underReplicatedBlocks.remove(ucBlock, replicas.liveReplicas(),
        replicas.decommissionedAndDecommissioning(), getReplication(ucBlock));
    pendingReplicationBlocks.remove(ucBlock);
    ...
    final long fileLength = bc.computeContentSummary(getBlockStoragePolicySuite()).getLength();
  ...
  private List<LocatedBlock> createLocatedBlockList(
      final BlockNeighborInfo[] blocks,
  ...
  private LocatedBlock createLocatedBlock(final BlockNeighborInfo[] blocks,
  ...
  private LocatedBlock createLocatedBlock(final BlockNeighborInfo blk, final long pos,
  ...
  private LocatedBlock createLocatedBlock(final BlockNeighborInfo blk, final long pos
    ...
    if (blk instanceof BlockNeighborInfoUnderConstruction) {
      if (blk.isComplete()) {
        throw new IOException(
            "blk instanceof BlockInfoUnderConstruction && blk.isComplete()"
            + ", blk=" + blk);
      }
      final BlockNeighborInfoUnderConstruction uc =
          (BlockNeighborInfoUnderConstruction) blk;
    ...
    // get block locations
    final int numCorruptNodes = getReplicaCount(blk).corruptReplicas();
    final int numCorruptReplicas = corruptReplicasMap.numCorruptReplicas(blk);
    ...
    if (numMachines > 0) {
      for(DatanodeStorageInfo storage : blocksMap.getStorageIterator(blk)) {
        ...
        final boolean replicaCorrupt = corruptReplicasMap.isReplicaCorrupt(blk, d);
  ...
  public LocatedBlocks createLocatedBlocks(final BlockNeighborInfo[] blocks,
    ...
    if (blocks == null) {
      ...
      if (!inSnapshot) {
        final BlockNeighborInfo last = blocks[blocks.length - 1];
  ...
  public short adjustReplication(short replication) {
    return replication < minReplication? minReplication
        : replication > maxReplicaCount ? maxReplicaCount : replication;
  }
   ...
   */
  public boolean isSufficientlyReplicated(BlockNeighborInfo b) {
    // Compare against the lesser of the minReplication and number of live DNs.
    final int replication =
        Math.min(minReplication, getDatanodeManager().getNumLiveDataNodes());
    return getReplicaCount(b).liveReplicas() >= replication;
  }
  ...
  private BlocksWithLocations getBlocksWithLocations(final DatanodeID datanode,
    ...
    Iterator<BlockNeighborInfo> iter = node.getBlockIterator(startBlock);
  ...
  void removeBlocksAssociatedTo(final DatanodeDescriptor node) {
    ...
    pendingDataNodeMessages.removeAllMessagesForDatanode(node);
  ...
  public void findAndMarkBlockAsCorrupt(final ExtendedBlock blk,
    ...
    assert namesystem.hasWriteLock();
    final BlockNeighborInfo storedBlock = getStoredBlock(blk.getLocalBlock());
    if (storedBlock == null) {
    ...
    markBlockAsCorrupt(new CorruptedBlockInfo(storedBlock,
   ...
   * @throws IOException
   */
  private void markBlockAsCorrupt(CorruptedBlockInfo b,
                                  DatanodeStorageInfo storageInfo,
                                  DatanodeDescriptor node) throws IOException {

    BlockSet bc = b.corrupted.getBlockSet();
    if (bc == null) {
    ...
    corruptReplicasMap.addToCorruptReplicasMap(b.corrupted, node, b.reason,
        b.reasonCode);

    ReplicaCount numberOfReplicas = getReplicaCount(b.stored);
   ...
   */
  private boolean invalidateBlock(CorruptedBlockInfo b, DatanodeInfo dn
      ) throws IOException {
    ...
    ReplicaCount nr = getReplicaCount(b.stored);
  ...
  void updateState() {
    pendingReplicationBlocksCount = pendingReplicationBlocks.size();
    underReplicatedBlocksCount = underReplicatedBlocks.size();
    corruptReplicaBlocksCount = corruptReplicasMap.size();
  }

  /** Return number of under-replicated but not missing blocks */
  public int getUnderReplicatedNotMissingBlocks() {
    return underReplicatedBlocks.getUnderReplicatedBlockCount();
  }
  ...
  /**
   * Scan blocks in {@link #underReplicatedBlocks} and assign replication
  ...
  int computeReplicationWork(int blocksToProcess) {
    ...
    try {
      ...
      blocksToReplicate = underReplicatedBlocks
  ...
  int computeReplicationWorkForBlocks(List<List<Block>> blocksToReplicate) {
        ...
        for (int priority = 0; priority < blocksToReplicate.size(); priority++) {
          for (Block block : blocksToReplicate.get(priority)) {
            ...
            if(bc == null || (bc.isUnderConstruction() && block.equals(bc.getLastBlock()))) {
              underReplicatedBlocks.remove(block, priority); // remove from neededReplications
              underReplicatedBlocks.decrementReplicationIndex(priority);
              continue;
            }
            ...
            numEffectiveReplicas = numReplicas.liveReplicas() +
                                    pendingReplicationBlocks.getNumReplicas(block);
      
            if (numEffectiveReplicas >= requiredReplication) {
              if ( (pendingReplicationBlocks.getNumReplicas(block) > 0) ||
    ...
    for(ReplicationWork rw : replicationWorks){
      ...
      rw.chooseTargets(blockPlacementPolicy, blockStoragePolicySuite, excludedNodes);
    }
    ...
    try {
      for(ReplicationWork rw : replicationWorks){
        ...
        synchronized (underReplicatedBlocks) {
          ...
          // do not schedule more if enough replicas is already pending
          ReplicaCount numReplicas = getReplicaCount(block);
          numEffectiveReplicas = numReplicas.liveReplicas() +
            pendingReplicationBlocks.getNumReplicas(block);
          ...
          pendingReplicationBlocks.increment(block,
    ...
    if (blockLog.isInfoEnabled()) {
      ...
      for(ReplicationWork rw : replicationWorks){
        ...
        if (targets != null && targets.length != 0) {
          ...
          blockLog.info("BLOCK* ask {} to replicate {} to {}", rw.getSrcNode(),
    ...
    if (blockLog.isDebugEnabled()) {
      blockLog.debug("BLOCK* neededReplications = {} pendingReplications = {}",
          underReplicatedBlocks.size(), pendingReplicationBlocks.size());
    }
  ...
  public DatanodeStorageInfo[] chooseTarget4WebHDFS(String src,
    ...
    return blockPlacementPolicy.chooseTarget(src, 1, clientnode,
  ...
  public DatanodeStorageInfo[] chooseTarget4AdditionalDatanode(String src,
    ...
    final BlockStoragePolicy storagePolicy = blockStoragePolicySuite.getPolicy(storagePolicyID);
    return blockPlacementPolicy.chooseTarget(src, numAdditionalNodes, clientnode,
  ...
  public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src,
    ...
    final BlockStoragePolicy storagePolicy = blockStoragePolicySuite.getPolicy(storagePolicyID);
    final DatanodeStorageInfo[] targets = blockPlacementPolicy.chooseTarget(src,
   ...
   */
   ...
   DatanodeDescriptor chooseSourceDatanode(Block block,
    ...
    Collection<DatanodeDescriptor> nodesCorrupt = corruptReplicasMap.getNodes(block);
    for(DatanodeStorageInfo storage : blocksMap.getStorageIterator(block)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
  ...
  private void processPendingReplications() {
    Block[] timedOutItems = pendingReplicationBlocks.getTimedOutBlocks();
    if (timedOutItems != null) {
      ...
      try {
        for (int i = 0; i < timedOutItems.length; i++) {
           ...
           */
          BlockNeighborInfo blockNeighborInfo = blocksMap.getBlockNeighborInfo(timedOutItems[i]);
          if (blockNeighborInfo == null) {
            continue;
          }
          ReplicaCount num = getReplicaCount(timedOutItems[i]);
          if (isNeededReplication(blockNeighborInfo, getReplication(blockNeighborInfo), num.liveReplicas())) {
            underReplicatedBlocks.add(blockNeighborInfo, num.liveReplicas(),
                num.decommissionedAndDecommissioning(), getReplication(blockNeighborInfo));
          }
        }
      } finally {
        namesystem.writeUnlock();
  ...
  static class StatefulBlockInfo {
    final BlockNeighborInfoUnderConstruction storedBlock;
    final Block reportedBlock;
    final ReplicaState reportedState;
    
    StatefulBlockInfo(BlockNeighborInfoUnderConstruction storedBlock,
  ...
  void rescanPostponedMisreplicatedBlocks() {
    ...
    try {
      ...
      for (int i=0; i < blocksPerPostpondedRescan && it.hasNext(); i++) {
        ...
        BlockNeighborInfo bi = blocksMap.getBlockNeighborInfo(b);
        if (bi == null) {
  ...
  private Collection<Block> processReport(
    ...
    //
    Collection<BlockNeighborInfo> toAdd = new LinkedList<BlockNeighborInfo>();
    Collection<Block> toRemove = new TreeSet<Block>();
    Collection<Block> toInvalidate = new LinkedList<Block>();
    Collection<CorruptedBlockInfo> toCorrupt = new LinkedList<CorruptedBlockInfo>();
    ...
    for (CorruptedBlockInfo b : toCorrupt) {
   ...
   * newStorages list.
   */
  public void markBlockReplicasAsCorrupt(BlockNeighborInfo block,
                                         long oldGenerationStamp, long oldNumBytes,
                                         DatanodeStorageInfo[] newStorages) throws IOException {
    assert namesystem.hasWriteLock();
    CorruptedBlockInfo b = null;
    if (block.getGenerationStamp() != oldGenerationStamp) {
      b = new CorruptedBlockInfo(block, oldGenerationStamp,
          "genstamp does not match " + oldGenerationStamp
          + " : " + block.getGenerationStamp(), Reason.GENSTAMP_MISMATCH);
    } else if (block.getNumBytes() != oldNumBytes) {
      b = new CorruptedBlockInfo(block,
  ...
  private void processFirstBlockReport(
    ...
    for (BlockReportReplica iblk : report) {
      ...
      BlockNeighborInfo storedBlock = blocksMap.getBlockNeighborInfo(iblk);
      ...
      if (isBlockUnderConstruction(storedBlock, ucState, reportedState)) {
        ((BlockNeighborInfoUnderConstruction)storedBlock)
            .addReplicaIfNotPresent(storageInfo, iblk, reportedState);
        // OpenFileBlocks only inside snapshots also will be added to safemode
        // threshold. So we need to update such blocks to safemode
        // refer HDFS-5283
        BlockNeighborInfoUnderConstruction blockUC =
            (BlockNeighborInfoUnderConstruction) storedBlock;
  ...
  private void reportDiff(DatanodeStorageInfo storageInfo,
                          ...
                          Collection<BlockNeighborInfo> toAdd,              // add to DatanodeDescriptor
                          Collection<Block> toRemove,           // remove from DatanodeDescriptor
                          Collection<Block> toInvalidate,       // should be removed from DN
                          Collection<CorruptedBlockInfo> toCorrupt, // add to corrupt replicas list
                          Collection<StatefulBlockInfo> toUC) { // add to under-construction list

    // place a delimiter in the list which separates blocks 
    // that have been reported from those that have not
    BlockNeighborInfo delimiter = new BlockNeighborInfo(new Block(), (short) 1);
   ...
   *         Otherwise, null.
   */
  private BlockNeighborInfo processReportedBlock(
      final DatanodeStorageInfo storageInfo,
      final Block block, final ReplicaState reportedState, 
      final Collection<BlockNeighborInfo> toAdd,
      final Collection<Block> toInvalidate, 
      final Collection<CorruptedBlockInfo> toCorrupt,
    ...
    BlockNeighborInfo blockNeighborInfo = blocksMap.getBlockNeighborInfo(block);
    if(blockNeighborInfo == null) {
    ...
    if (reportedState == ReplicaState.FINALIZED
        && (blockNeighborInfo.findStorageInfo(storageInfo) == -1 ||
            corruptReplicasMap.isReplicaCorrupt(blockNeighborInfo, datanodeDescriptor))) {
      toAdd.add(blockNeighborInfo);
    }
    return blockNeighborInfo;
  }
  ...
  private void queueReportedBlock(DatanodeStorageInfo storageInfo, Block block,
    ...
    pendingDataNodeMessages.enqueueReportedBlock(storageInfo, block, reportedState);
  }
  ...
  public void processQueuedMessagesForBlock(Block b) throws IOException {
    Queue<ReportedBlockInfo> queue = pendingDataNodeMessages.takeBlockQueue(b);
    if (queue == null) {
  ...
  public void processAllPendingDNMessages() throws IOException {
    assert !shouldPostponeBlocksFromFuture :
      ...
      "block postponement.";
    int count = pendingDataNodeMessages.count();
    if (count > 0) {
      LOG.info("Processing " + count + " messages from DataNodes " +
          "that were previously queued during standby state");
    }
    processQueuedMessages(pendingDataNodeMessages.takeAll());
    assert pendingDataNodeMessages.count() == 0;
  }
   ...
   */
  private CorruptedBlockInfo checkReplicaCorrupt(
          Block reported, ReplicaState reportedState,
          BlockNeighborInfo storedBlock, BlockUCState ucState,
    ...
    case FINALIZED:
      ...
      case COMMITTED:
        if (storedBlock.getGenerationStamp() != reported.getGenerationStamp()) {
          ...
          return new CorruptedBlockInfo(storedBlock, reportedGS,
              "block is " + ucState + " and reported genstamp " + reportedGS
              + " does not match genstamp in block map "
              + storedBlock.getGenerationStamp(), Reason.GENSTAMP_MISMATCH);
        } else if (storedBlock.getNumBytes() != reported.getNumBytes()) {
          return new CorruptedBlockInfo(storedBlock,
      ...
      case UNDER_CONSTRUCTION:
        if (storedBlock.getGenerationStamp() > reported.getGenerationStamp()) {
          final long reportedGS = reported.getGenerationStamp();
          return new CorruptedBlockInfo(storedBlock, reportedGS, "block is "
    ...
    default:
      ...
      LOG.warn(msg);
      return new CorruptedBlockInfo(storedBlock, msg, Reason.INVALID_STATE);
    }
  }

  private boolean isBlockUnderConstruction(BlockNeighborInfo storedBlock,
  ...
  void addStoredBlockUnderConstruction(StatefulBlockInfo ucBlock,
    ...
    BlockNeighborInfoUnderConstruction block = ucBlock.storedBlock;
   ...
   * @throws IOException
   */
  private void addStoredBlockImmediate(BlockNeighborInfo storedBlock,
                                       DatanodeStorageInfo storageInfo)
  throws IOException {
    ...
    if (storedBlock.getBlockUCState() == BlockUCState.COMMITTED
      ...
      completeBlock(storedBlock.getBlockSet(), storedBlock, null, false);
   ...
   */
  private Block addStoredBlock(final BlockNeighborInfo block,
                               DatanodeStorageInfo storageInfo,
  ...
  throws IOException {
    ...
    if (block instanceof BlockNeighborInfoUnderConstruction) {
      ...
      storedBlock = blocksMap.getBlockNeighborInfo(block);
    } else {
      storedBlock = block;
    }
    if (storedBlock == null || storedBlock.getBlockSet() == null) {
    ...
    BlockSet bc = storedBlock.getBlockSet();
    ...
    if (result == AddBlockResult.ADDED) {
      ...
      // corrupt block list.
      corruptReplicasMap.removeFromCorruptReplicasMap(block, node,
  ...
  private void logAddStoredBlock(BlockNeighborInfo storedBlock,
   ...
   */
  private void invalidateCorruptReplicas(BlockNeighborInfo blk) {
    Collection<DatanodeDescriptor> nodes = corruptReplicasMap.getNodes(blk);
    ...
    for (DatanodeDescriptor node : nodesCopy) {
      try {
        if (!invalidateBlock(new CorruptedBlockInfo(blk, null,
              Reason.ANY), node)) {
    ...
    if (removedFromBlocksMap) {
      corruptReplicasMap.removeFromCorruptReplicasMap(blk);
    }
  }
    ...
    while (namesystem.isRunning() && !Thread.currentThread().isInterrupted()) {
      ...
      try {
        while (processed < numBlocksPerIteration && blocksItr.hasNext()) {
          BlockNeighborInfo block = blocksItr.next();
   ...
   */
  private MisReplicationResult processMisReplicatedBlock(BlockNeighborInfo block) {
    BlockSet bc = block.getBlockSet();
    if (bc == null) {
    ...
    ReplicaCount num = getReplicaCount(block);
    int numCurrentReplica = num.liveReplicas();
    // add to under-replicated queue if need to be
    if (isNeededReplication(block, expectedReplication, numCurrentReplica)) {
      if (underReplicatedBlocks.add(block, numCurrentReplica, num
  ...
  private void processOverReplicatedBlock(final Block block,
    ...
    Collection<DatanodeDescriptor> corruptNodes = corruptReplicasMap
        .getNodes(block);
  ...
  private void chooseExcessReplicates(
    ...
    BlockSet bc = getBlockSet(b);
    final BlockStoragePolicy storagePolicy = blockStoragePolicySuite.getPolicy(
        bc.getStoragePolicyID());
    final List<StorageType> excessTypes = storagePolicy.chooseExcess(
        replication, DatanodeStorageInfo.toStorageTypes(nonExcess));
    List<DatanodeStorageInfo> replicasToDelete = blockPlacementPolicy
  ...
  private void removeStaleReplicas(List<ReplicaUnderConstruction> staleReplicas,
      BlockNeighborInfoUnderConstruction block) {
  ...
  void addBlock(DatanodeStorageInfo storageInfo, Block block, String delHint)
    ...
    //
    pendingReplicationBlocks.decrement(block, node);
   ...
   */
  public ReplicaCount getReplicaCount(Block block) {
    int decommissioned = 0;
    int decommissioning = 0;
    int live = 0;
    int corrupt = 0;
    int excess = 0;
    int stale = 0;
    Collection<DatanodeDescriptor> corruptedBlockNodes = corruptReplicasMap.getNodes(block);
    for(DatanodeStorageInfo storage : blocksMap.getStorageIterator(block, State.NORMAL)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
      if ((corruptedBlockNodes != null) && (corruptedBlockNodes.contains(node))) {
        corrupt++;
      } else if (node.isDecommissionInProgress()) {
        decommissioning++;
      } else if (node.isDecommissioned()) {
    ...
    return new ReplicaCount(live, decommissioned, decommissioning, corrupt, excess, stale);
  }

  /** 
   * Simpler, faster form of {@link #getReplicaCount(Block)} that only returns the number
   * of live nodes.  If in startup safemode (or its 30-sec extension period),
   * then it gains speed by ignoring issues of excess replicas or nodes
   * that are decommissioned or in process of becoming decommissioned.
   * If not in startup, then it calls {@link #getReplicaCount(Block)} instead.
   * 
   * @param b - the block being tested
   * @return count of live nodes for this block
   */
  int countLiveNodes(BlockNeighborInfo b) {
    if (!namesystem.isInStartupSafeMode()) {
      return getReplicaCount(b).liveReplicas();
    }
    // else proceed with fast case
    int live = 0;
    Collection<DatanodeDescriptor> nodesCorrupt = corruptReplicasMap.getNodes(b);
    for(DatanodeStorageInfo storage : blocksMap.getStorageIterator(b, State.NORMAL)) {
      final DatanodeDescriptor node = storage.getDatanodeDescriptor();
      if ((nodesCorrupt == null) || (!nodesCorrupt.contains(node)))
        live++;
  ...
  public void removeBlock(Block block) {
    ...
    underReplicatedBlocks.remove(block, UnderReplicatedBlocks.LEVEL);
    postponedMisreplicatedBlocks.remove(block);
  }

  public BlockNeighborInfo getStoredBlock(Block block) {
    return blocksMap.getBlockNeighborInfo(block);
  }

  public void updateLastBlock(BlockNeighborInfoUnderConstruction lastBlock,
  ...
  private void updateNeededReplications(final Block block,
    ...
    try {
      ...
      if (isNeededReplication(block, curExpectedReplicas, repl.liveReplicas())) {
        underReplicatedBlocks.update(block, repl.liveReplicas(), repl
            .decommissionedAndDecommissioning(), curExpectedReplicas,
            curReplicasDelta, expectedReplicasDelta);
      } else {
        int oldReplicas = repl.liveReplicas()-curReplicasDelta;
        int oldExpectedReplicas = curExpectedReplicas-expectedReplicasDelta;
        underReplicatedBlocks.remove(block, oldReplicas,
   ...
   */
  public void checkReplication(BlockSet bc) {
    final short expected = bc.getBlockReplication();
    for (Block block : bc.getBlockNeighborInfos()) {
      final ReplicaCount n = getReplicaCount(block);
      if (isNeededReplication(block, expected, n.liveReplicas())) { 
        underReplicatedBlocks.add(block, n.liveReplicas(),
  ...
  private int getReplication(Block block) {
    final BlockSet blockSet = blocksMap.getBlockSet(block);
    return blockSet == null? 0: blockSet.getBlockReplication();
  }
  ...
  private int invalidateWorkForOneNode(DatanodeInfo datanodeInfo) {
    ...
    try {
      ...
      try {
        DatanodeDescriptor datanodeDescriptor = datanodeManager.getDatanode(datanodeInfo);
        if (datanodeDescriptor == null) {
          LOG.warn("DataNode " + datanodeInfo + " cannot be found with UUID " +
              datanodeInfo.getDatanodeUuid() + ", removing block invalidation work.");
          invalidateBlocks.remove(datanodeInfo);
          return 0;
        }
        toInvalidate = invalidateBlocks.invalidateWork(datanodeDescriptor);
    ...
    blockLog.info("BLOCK* {}: ask {} to delete {}", getClass().getSimpleName(),
        datanodeInfo, toInvalidate);
    return toInvalidate.size();
  }

  boolean isPlacementPolicySatisfied(Block block) {
    List<DatanodeDescriptor> liveNodes = new ArrayList<>();
    Collection<DatanodeDescriptor> corruptNodes = corruptReplicasMap
        .getNodes(block);
    for (DatanodeStorageInfo datanodeStorageInfo : blocksMap.getStorageIterator(block)) {
      final DatanodeDescriptor datanodeDescriptor = datanodeStorageInfo.getDatanodeDescriptor();
      if (!datanodeDescriptor.isDecommissionInProgress() && !datanodeDescriptor.isDecommissioned()
          && ((corruptNodes == null) || !corruptNodes.contains(datanodeDescriptor))) {
        liveNodes.add(datanodeDescriptor);
      }
    }
    DatanodeInfo[] locs = liveNodes.toArray(new DatanodeInfo[liveNodes.size()]);
    return blockPlacementPolicy.verifyBlockPlacement(locs,
        getReplication(block)).isPlacementPolicySatisfied();
  }
   ...
   */
  boolean isNeededReplication(Block block, int expected, int current) {
    BlockNeighborInfo blockInfo;
    if (block instanceof BlockNeighborInfo) {
      blockInfo = (BlockNeighborInfo) block;
    } else {
      blockInfo = getStoredBlock(block);
    }
    return blockInfo.isComplete()
        && (current < expected || !isPlacementPolicySatisfied(block));
  }
  
  public long getMissingBlocksCount() {
    // not locking
    return this.underReplicatedBlocks.getCorruptBlockSize();
  }

  public long getMissingReplicationOneBlocksCount() {
    // not locking
    return this.underReplicatedBlocks.getCorruptReplOneBlockSize();
  }
  ...
  public int numCorruptReplicas(Block block) {
    return corruptReplicasMap.numCorruptReplicas(block);
  }
  ...
  public void removeBlockFromMap(Block block) {
    ...
    corruptReplicasMap.removeFromCorruptReplicasMap(block);
  }
  ...
  public long[] getCorruptReplicaBlockIds(int numExpectedBlocks,
    ...
    return corruptReplicasMap.getCorruptReplicaBlockIds(numExpectedBlocks,
                                                     startingBlockId);
  }
  ...
  public Iterator<Block> getCorruptReplicaBlockIterator() {
    return underReplicatedBlocks.iterator(
  ...
  public Collection<DatanodeDescriptor> getCorruptReplicas(Block block) {
    return corruptReplicasMap.getNodes(block);
  }
 ...
 public String getCorruptReason(Block block, DatanodeDescriptor node) {
   return corruptReplicasMap.getCorruptReason(block, node);
 }

  /** @return the size of UnderReplicatedBlocks */
  public int numOfUnderReplicatedBlocks() {
    return underReplicatedBlocks.size();
  }
  ...
  int computeDatanodeWork() {
    ...
    final int liveDatanodeCount = heartbeatManager.getLiveDatanodeCount();
    final int blocksToProcess = liveDatanodeCount
        * this.blocksReplWorkMultiplier;
    final int nodesToProcess = (int) Math.ceil(liveDatanodeCount
  ...
  public void enqueueBlockOp(final Runnable runnable) throws IOException {
    try {
      blockReportProcessingThread.enqueue(runnable);
    } catch (InterruptedException ie) {
  ...
  public int getBlockOpQueueLength() {
    return blockReportProcessingThread.runnableBlockingQueue.size();
  }
    ...
    private void processQueue() {
      while (namesystem.isRunning()) {
        ...
        try {
          ...
          int processed = 0;
          namesystem.writeLock();
          metrics.setBlockOpsQueued(runnableBlockingQueue.size() + 1);
          try {
            ...
            do {
              ...
              runnable = runnableBlockingQueue.poll();
            } while (runnable != null);
          } finally {
            namesystem.writeUnlock();
  ...
  public int getPendingDataNodeMessageCount() {
    return pendingDataNodeMessages.count();
  }
