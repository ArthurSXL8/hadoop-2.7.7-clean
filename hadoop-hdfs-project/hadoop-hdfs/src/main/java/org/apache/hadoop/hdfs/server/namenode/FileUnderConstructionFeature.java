...
public class FileUnderConstructionFeature implements INode.Feature {
  ...
  void updateLengthOfLastBlock(INodeFile f, long lastBlockLength)
      throws IOException {
    BlockNeighborInfo lastBlock = f.getLastBlock();
    assert (lastBlock != null) : "The last block for path "
        + f.getFullPathName() + " is null when updating its length";
    assert (lastBlock instanceof BlockNeighborInfoUnderConstruction)
  ...
  void cleanZeroSizeBlock(final INodeFile f,
    ...
    if (blocks != null && blocks.length > 0
      ...
      BlockNeighborInfoUnderConstruction lastUC =
          (BlockNeighborInfoUnderConstruction) blocks[blocks.length - 1];
