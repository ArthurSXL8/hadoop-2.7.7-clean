...
public final class FSImageFormatPBINode {
  ...
  public final static class Loader {
    ...
    private INodeFile loadINodeFile(INodeSection.INode n) {
      ...
      BlockNeighborInfo[] blocks = new BlockNeighborInfo[bp.size()];
      for (int i = 0, e = bp.size(); i < e; ++i) {
        blocks[i] = new BlockNeighborInfo(PBHelper.convert(bp.get(i)), replication);
      }
      ...
      if (f.hasFileUC()) {
        ...
        if (blocks.length > 0) {
          BlockNeighborInfo lastBlk = file.getLastBlock();
          // replace the last block of file
          file.setBlock(file.numBlocks() - 1, new BlockNeighborInfoUnderConstruction(
              lastBlk, replication));
        }
      }
      return file;
