...
public class FSEditLogLoader {
  ...
  private void addNewBlock(FSDirectory fsDir, AddBlockOp op, INodeFile file)
    ...
    if (pBlock != null) { // the penultimate block is not null
      ...
      if (oldLastBlock instanceof BlockNeighborInfoUnderConstruction) {
        fsNamesys.getBlockManager().forceCompleteBlock(file,
            (BlockNeighborInfoUnderConstruction) oldLastBlock);
  ...
  private void updateBlocks(FSDirectory fsDir, BlockListUpdatingOp op,
    ...
    for (int i = 0; i < oldBlocks.length && i < newBlocks.length; i++) {
      BlockNeighborInfo oldBlock = oldBlocks[i];
      ...
      if (oldBlock instanceof BlockNeighborInfoUnderConstruction &&
          (!isLastBlock || op.shouldCompleteLastBlock())) {
        changeMade = true;
        fsNamesys.getBlockManager().forceCompleteBlock(file,
            (BlockNeighborInfoUnderConstruction) oldBlock);
      }
      if (changeMade) {
    ...
    if (newBlocks.length < oldBlocks.length) {
      ...
      for (int i = oldBlocks.length; i < newBlocks.length; i++) {
        Block newBlock = newBlocks[i];
        BlockNeighborInfo newBI;
        if (!op.shouldCompleteLastBlock()) {
          // TODO: shouldn't this only be true for the last block?
          // what about an old-version fsync() where fsync isn't called
          // until several blocks in?
          newBI = new BlockNeighborInfoUnderConstruction(
              newBlock, file.getBlockReplication());
        } else {
